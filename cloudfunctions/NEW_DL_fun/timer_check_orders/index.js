...
// 未传 envVersion 时默认按 develop 处理（与 dev_index.js 中环境默认值一致）。
// 注意：本函数由定时触发器调用，事件不携带 envVersion，因此固定走 develop→dev_index.js 分支；
// 当前 dev_index.js 与 true_index.js 内容一致、行为无差异。若日后修改 dev 版本，
// 请记得执行同步脚本把 dev_index.js 覆盖到 true_index.js，保证定时任务跑的是确认过的代码。
