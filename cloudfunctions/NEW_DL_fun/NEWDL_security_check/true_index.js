...
// 新增运行环境日志：用于快速判断当前安全检查云函数这次按什么环境、什么源码文件在执行
function logRuntimeEnvInfo(extra = {}) {
  console.log('[runtime_env]', {
    functionName: 'NEWDL_security_check',
...
function buildTraceId(action = '') {
  return `security_check_${String(action || 'unknown')}_${Date.now()}_${Math.floor(Math.random() * 10000)}`
}
...
async function checkTextSecurity(content = '', openid = '', traceId = '') {
  ...
  if (!text) {
    console.log('[NEWDL_security_check] [TEXT_SKIP_EMPTY]', {
      traceId,
  ...
  try {
    console.log('[NEWDL_security_check] [TEXT_START]', {
      traceId,
      openid: maskOpenid(openid),
    ...
    if (isSecurityCheckPassed(result)) {
      console.log('[NEWDL_security_check] [TEXT_PASS]', {
        traceId,
        openid: maskOpenid(openid),
        textLength: text.length
      })
    ...
    console.warn('[NEWDL_security_check] [TEXT_BLOCKED]', {
      traceId,
      openid: maskOpenid(openid),
      textLength: text.length,
      result
    ...
    if (isSecurityViolationError(error)) {
      console.warn('[NEWDL_security_check] [TEXT_VIOLATION]', {
        traceId,
        openid: maskOpenid(openid),
        textLength: text.length,
    ...
    console.error('[NEWDL_security_check] [TEXT_ERROR]', {
      traceId,
      openid: maskOpenid(openid),
      textLength: text.length,
      error: normalizeErrorLog(error)
...
async function checkImageSecurity(fileID = '', traceId = '') {
  ...
  if (!trimmedFileId) {
    console.log('[NEWDL_security_check] [IMAGE_SKIP_EMPTY]', {
      traceId
    })
    return buildSafeResult({
      checkType: 'image',
  ...
  try {
    console.log('[NEWDL_security_check] [IMAGE_DOWNLOAD_START]', {
      traceId,
    ...
    console.log('[NEWDL_security_check] [IMAGE_DOWNLOAD_SUCCESS]', {
      traceId,
      fileID: summarizeFileID(trimmedFileId),
    ...
    if (isSecurityCheckPassed(result)) {
      console.log('[NEWDL_security_check] [IMAGE_PASS]', {
        traceId,
        fileID: summarizeFileID(trimmedFileId)
      })
      return buildSafeResult({
    ...
    console.warn('[NEWDL_security_check] [IMAGE_BLOCKED]', {
      traceId,
      fileID: summarizeFileID(trimmedFileId),
      result
    })
    ...
    if (isSecurityViolationError(error)) {
      console.warn('[NEWDL_security_check] [IMAGE_VIOLATION]', {
        traceId,
        fileID: summarizeFileID(trimmedFileId),
        error: normalizeErrorLog(error)
      })
    ...
    if (isImageSizeLimitError(error)) {
      console.warn('[NEWDL_security_check] [IMAGE_TOO_LARGE]', {
        traceId,
        fileID: summarizeFileID(trimmedFileId),
        error: normalizeErrorLog(error)
      })
    ...
    console.error('[NEWDL_security_check] [IMAGE_ERROR]', {
      traceId,
      fileID: summarizeFileID(trimmedFileId),
      error: normalizeErrorLog(error)
    })
...
async function validateProfileSecurity(profile = {}, openid = '', traceId = '') {
  ...
  console.log('[NEWDL_security_check] [PROFILE_VALIDATE_START]', {
    traceId,
    openid: maskOpenid(openid),
  ...
  for (let index = 0; index < textList.length; index += 1) {
    console.log('[NEWDL_security_check] [PROFILE_TEXT_ITEM_START]', {
      traceId,
      openid: maskOpenid(openid),
      index,
    ...
    if (!textResult.safe) {
      console.warn('[NEWDL_security_check] [PROFILE_TEXT_ITEM_BLOCKED]', {
        traceId,
        openid: maskOpenid(openid),
        index,
        result: textResult
  ...
  for (let index = 0; index < imageList.length; index += 1) {
    console.log('[NEWDL_security_check] [PROFILE_IMAGE_ITEM_START]', {
      traceId,
      openid: maskOpenid(openid),
      index,
    ...
    if (!imageResult.safe) {
      console.warn('[NEWDL_security_check] [PROFILE_IMAGE_ITEM_BLOCKED]', {
        traceId,
        openid: maskOpenid(openid),
        index,
        result: imageResult
  ...
  console.log('[NEWDL_security_check] [PROFILE_VALIDATE_PASS]', {
    traceId,
    openid: maskOpenid(openid),
    textCount: textList.length,
    imageCount: imageList.length
...
exports.main = async (event, context) => {
  CURRENT_ENV_VERSION = (event && event.envVersion) || 'develop'
  ...
  try {
    ...
    console.log('[NEWDL_security_check] [ENTER]', {
      traceId,
      action,
    ...
    switch (action) {
      ...
      default:
        console.warn('[NEWDL_security_check] [UNKNOWN_ACTION]', {
          traceId,
          action,
          operatorOpenid: maskOpenid(operatorOpenid)
        })
    ...
    const permissionMessage = isOpenAPIPermissionError(error)
      ? 'NEWDL_security_check 缺少 OpenAPI 权限，请重新上传云函数并确认 config.json 已生效'
      : '安全检查失败'
    console.error('NEWDL_security_check 执行失败', {
      traceId,
      action,
      operatorOpenid: maskOpenid(operatorOpenid),
      error: normalizeErrorLog(error)
